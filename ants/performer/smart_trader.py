...
class SmartTrader:
    ...
    def trading(
        ...
        self.logger.debug(
            "trading with {} {} {} {} {} {}".format(
                exchange_name, market, action, coin_name, price, amount
            )
        )
    ...
    def _buy(
        self, exchange, market, coin_name, amount, price=None, etc={}, want_market_price=False
    ):
        symbol = coin_name + "/" + market  #'BTC/KRW'
        ...
        if price >= last_price * 1.5:
            raise Exception(
                "구매단가가 너무 높습니다.(현재가 1.5배 초과) 주문을 취소합니다.\n현재가:{}\n주문가:{}".format(last_price, price)
            )
    ...
    def _sell(
        self, exchange, market, coin_name, price=None, amount=None, etc={}, want_market_price=False
    ):
        symbol = coin_name + "/" + market  #'BTC/KRW'
        _type = "market" if want_market_price else "limit"  # or 'market' or 'limit'
        ...
        if price <= last_price * 0.5:
            raise Exception(
                "판매단가가 너무 낮습니다.(현재가의 절반 미만) 주문을 취소합니다.\n현재가:{}\n주문가:{}".format(last_price, price)
            )
