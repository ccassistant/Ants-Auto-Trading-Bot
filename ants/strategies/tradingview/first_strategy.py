...
class EmailAlretStrategy(ants.strategies.strategy.StrategyBase, Observer):
    ...
    def __init__(self, args={}):
        ...
        self.upbit = cUpbit(
            {
                "root_config_file": "configs/ants.conf",
                "key_file": "configs/exchanges.key",
                "config_file": "configs/upbit.conf",
    ...
    def do_action(self, msg):
        ...
        for order in orders:
            if order.get()["symbol"] == symbol and order.get()["side"] == "buy":
                self.logger.info(
                    "'BUY' Signal will ignore cause old order is not fill {}, ts:{}".format(
                        symbol, order.get()["ts_create"]
                    )
                )
                return
        ...
        if action.upper() == "BUY":
            if availabel_size > 0:
                self.logger.info(
                    "'BUY' Signal will ignore cause balance is enought {} {}".format(
                        coin_name, availabel_size
                    )
                )
                return
    ...
    def old_do_action(self, msg):
        ...
        if self.get_state(exchange, coin_name, market) == action:
            self.logger.info(
                "Already {} {}/{} {} state".format(exchange, coin_name, market, action)
            )
            return
    ...
    def save_state(self, exchange, coin, market, action):
        record = {"coin": coin, "market": market, "exchange": exchange, "action": action}
        """
        {
            'upbit': {
                'xrp': {
