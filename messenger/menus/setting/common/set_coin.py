...
class SetCoinList(MenuItem):
    ...
    def run(self):
        if Enviroments().exchanges.get(self.exchange_name).get("trading_list").get("all") == True:
            ...
            coin_list = (
                Enviroments().exchanges.get(self.exchange_name).get("trading_list").get("list")
            )
            if len(coin_list) == 0:
                msg = "{} 거래소 자동 매매  코인 목록 설정 목록 : \n{}".format(self.exchange_name.upper(), "없음")
            else:
                msg = "{} 거래소 자동 매매  코인 목록 설정 목록 : \n{}".format(
                    self.exchange_name.upper(), coin_list
                )
...
class CoinAdd(MenuItem):
    ...
    def __get_msg__(self):
        ...
        """.format(
            self.exchange_name.upper(), coins
        )
    ...
    def menu_keyboard(self):
        keyboard = [
            [
                InlineKeyboardButton("모든 코인 선택", callback_data="all_coin"),
...
class CoinDel(MenuItem):
    ...
    def edit_message(self, bot, query, msg):
        bot.edit_message_text(
            chat_id=query.message.chat_id, message_id=query.message.message_id, text=msg
        )
